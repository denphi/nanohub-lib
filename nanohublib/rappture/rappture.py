...
class RapXML(Node):
    ...
    def __init__(self, fname):
        ...
        if hasattr(self, 'dirname') and self.dirname:
             self._load_loaders()

        RapLoader.copy_defaults(self.tree, reset=False)
        self.top = self
    ...
    def _ipython_display_(self):
        if self.info.in_df.size:
            display(Markdown('## INPUTS'), self.info.in_df)
        if self.info.loader_df.size:
            display(Markdown('## LOADERS'), self.info.loader_df)
        if self.info.out_df.size:
            display(Markdown('## OUTPUTS'), self.info.out_df)
    ...
    def inputs(self):
        ...
        if self.info.loader_df.size:
            display(Markdown('## LOADERS'), self.info.loader_df)
        return self.info.in_df

    @property
    def outputs(self):
        display(self.info.out_df)        
        return self.info.out_df

    @property
    def loaders(self):
        display(self.info.loader_df)
        return self.info.loader_df
